...
async function saveComposeDraft(close=true){ const p=composePayload(); if (!p.to && !p.subject && !p.body) { composeFeedback('Add a recipient, subject, or message before saving.'); return; } const row={ client_id:p.clientId||null, to_email:p.to, cc:p.cc, bcc:p.bcc, subject:p.subject, body:p.body, body_html:p.bodyHtml, updated_at:new Date().toISOString() }; const res=p.draftId ? await sb.from('message_drafts').update(row).eq('id',p.draftId) : await sb.from('message_drafts').insert(row); if (res.error){ composeFeedback('Could not save draft. Apply migration 022 to enable drafts.'); return; } await loadDrafts(); if (close) closeModal(); toast('Draft saved.'); renderMessages(); }
async function deleteDraft(id){ await sb.from('message_drafts').delete().eq('id',id); if (selectedMessageId===`draft:${id}`) selectedMessageId=null; await loadDrafts(); renderMessages(); }
...
async function sendComposed(){
  ...
  let status='sent', errorText=null; try { const res=await fetch('/api/send-message',{ method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ to:p.to, cc:p.cc, bcc:p.bcc, subject:p.subject, body:p.body, htmlBody:p.bodyHtml, attachments }) }); if (!res.ok) { const fail=await res.json().catch(()=>({})); status='failed'; errorText=fail.error || `Send failed (${res.status})`; } } catch { status='failed'; errorText='Network error - message not sent.'; }
