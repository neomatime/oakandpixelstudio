...
let composeFiles = [];
let composeOriginalAttachments = [];
let composeSendId = '';
...
const COMPOSE_MAX_FILE_BYTES = 10 * 1024 * 1024;
const COMPOSE_MAX_TOTAL_BYTES = 25 * 1024 * 1024;
const COMPOSE_FILE_ACCEPT = '.pdf,.png,.jpg,.jpeg,.gif,.webp,.svg,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.csv,.txt,.zip';
const COMPOSE_ALLOWED_EXTENSIONS = new Set(COMPOSE_FILE_ACCEPT.split(',').map(value => value.slice(1)));
...
function normalizeDraft(row){ return { id:row.id, clientId:row.client_id||null, to:row.to_email||'', cc:row.cc||'', bcc:row.bcc||'', subject:row.subject||'', body:row.body||'', bodyHtml:row.body_html||'', attachments:Array.isArray(row.attachments)?row.attachments:[], createdAt:row.created_at, updatedAt:row.updated_at }; }
async function loadDrafts(){ const { data, error } = await sb.from('message_drafts').select('*').order('updated_at', { ascending:false }); allDrafts = error ? [] : (data||[]).map(normalizeDraft); }
function normalizeTask(row){ return { id:row.id, title:row.title||'Untitled task', status:row.status||'todo', priority:row.priority||'Medium', dueDate:row.due_date||'', emailId:row.email_id||'', notes:row.notes||'', updatedAt:row.updated_at }; }
async function loadTasks(){ const { data, error } = await sb.from('comm_tasks').select('*').order('updated_at', { ascending:false }); allTasks = error ? [] : (data||[]).map(normalizeTask); }
...
function draftById(id){ return allDrafts.find(d => String(d.id) === String(id)); }
function allMailItems(){
  const messages = allMessages.map(m => ({ type:'message', id:String(m.id), data:m, date:messageDate(m) }));
  const drafts = allDrafts.map(d => ({ type:'draft', id:`draft:${d.id}`, data:d, date:d.updatedAt || d.createdAt }));
...
function renderMailCounts(){
  ...
  set('mail-all-count', allMessages.length + allDrafts.length);
  set('mail-inbox-count', allMessages.filter(m => (m.direction || 'outbound') === 'inbound' && !metaFor(m.id).archived).length);
  set('mail-unread-count', unreadCount()); set('mail-starred-count', starredCount()); set('mail-important-count', importantCount()); set('mail-archived-count', archivedCount()); set('mail-drafts-count', allDrafts.length);
  const badge=$('messages-badge'); if (badge) { const n=unreadCount(); badge.textContent=n>9?'9+':n; badge.classList.toggle('show', n>0); }
}
function kpisHTML(){ const activeTasks=allTasks.filter(t => t.status !== 'completed').length; return `<div class="comm-kpis"><div class="comm-kpi"><span>${unreadCount()}</span><small>Unread</small></div><div class="comm-kpi"><span>${importantCount()}</span><small>Important</small></div><div class="comm-kpi"><span>${activeTasks}</span><small>Open Tasks</small></div><div class="comm-kpi"><span>${allAttachments().length}</span><small>Files</small></div></div>`; }
...
function mailBodyHTML(m){
  ...
  if (html){
    const doc = `<!DOCTYPE html><html><head><base target="_blank"><meta charset="utf-8"><style>html,body{margin:0;padding:14px;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#1a1a18;background:#fff;font-size:14px;line-height:1.6;}img{max-width:100%;height:auto;}a{color:#1A5C3A;}table{max-width:100%;}</style></head><body>${m.body_html}</body></html>`;
    return `<iframe class="mail-html-body" sandbox="allow-same-origin" referrerpolicy="no-referrer" onload="sizeMailFrame(this)" srcdoc="${esc(doc)}"></iframe>`;
  }
...
function composePayload(){ const bodyEl=$('mail-compose-body'); const bodyHtml=appendOpsEmailSignature(bodyEl?.innerHTML || ''); return { draftId:$('comm-compose-draft-id')?.value || '', clientId:$('mail-compose-client')?.value || null, to:($('mail-compose-to')?.value || '').trim(), cc:($('mail-compose-cc')?.value || '').trim(), bcc:($('mail-compose-bcc')?.value || '').trim(), subject:($('mail-compose-subject')?.value || '').trim(), bodyHtml, body:textWithoutOpsEmailSignature(bodyHtml) }; }
...
async function saveComposeDraft(close=true){ const p=composePayload(); if (!p.to && !p.subject && !p.body && !composeFiles.length) { composeFeedback('Add a recipient, subject, message, or attachment before saving.'); return; } const btn=$('comm-save-draft'); btnLoad(btn,true,'Saving...'); let uploadResult={ attachments:[], newPaths:[] }; try { const auth=await composeAuth(); uploadResult=await uploadComposeAttachments(auth.user.id); } catch(err) { btnLoad(btn,false); composeFeedback(err.message); return; } const row={ client_id:p.clientId||null, to_email:p.to, cc:p.cc, bcc:p.bcc, subject:p.subject, body:p.body, body_html:p.bodyHtml, attachments:uploadResult.attachments, updated_at:new Date().toISOString() }; const res=p.draftId ? await sb.from('message_drafts').update(row).eq('id',p.draftId) : await sb.from('message_drafts').insert(row); if (res.error){ await removeStoredAttachments(uploadResult.newPaths); btnLoad(btn,false); composeFeedback('Could not save this draft. Please try again.'); return; } await removeStoredAttachments(removedOriginalAttachmentPaths(uploadResult.attachments)); composeFiles=uploadResult.attachments.map(file=>({ ...file, kind:'stored' })); composeOriginalAttachments=composeFiles.map(file=>({ ...file })); await loadDrafts(); btnLoad(btn,false); if (close) closeModal(); toast('Draft saved.'); renderMessages(); }
async function deleteDraft(id, options={}){ const draft=draftById(id); const result=await sb.from('message_drafts').delete().eq('id',id); if (!result.error && !options.preserveAttachments) await removeStoredAttachments((draft?.attachments||[]).map(file=>file.path)); if (selectedMessageId===`draft:${id}`) selectedMessageId=null; await loadDrafts(); if (options.render!==false) renderMessages(); }
function formatFileSize(bytes=0){ const value=Number(bytes)||0; if (value<1024) return `${value} B`; if (value<1024*1024) return `${(value/1024).toFixed(value<10240?1:0)} KB`; return `${(value/1024/1024).toFixed(1)} MB`; }
function composeFileName(entry){ return entry.name || entry.filename || entry.file?.name || 'Attachment'; }
function composeFileSize(entry){ return Number(entry.size ?? entry.file?.size ?? 0); }
function composeFileExtension(entry){ return (composeFileName(entry).split('.').pop() || 'FILE').toUpperCase().slice(0,5); }
function composeFileKey(entry){ return entry.path || `${composeFileName(entry).toLowerCase()}:${composeFileSize(entry)}`; }
function renderComposeFiles(){ const list=$('comm-compose-file-list'); if (!list) return; if (!composeFiles.length) { list.innerHTML='<span class="compose-file-empty">No attachments added.</span>'; return; } const total=composeFiles.reduce((sum,file)=>sum+composeFileSize(file),0); list.innerHTML=`<div class="compose-file-summary"><span>${composeFiles.length} attachment${composeFiles.length===1?'':'s'}</span><span>${formatFileSize(total)} total</span></div>${composeFiles.map((file,i)=>`<div class="compose-file-chip"><span class="compose-file-type">${esc(composeFileExtension(file))}</span><span class="compose-file-copy"><strong>${esc(composeFileName(file))}</strong><small>${formatFileSize(composeFileSize(file))}${file.kind==='stored'?' - saved with draft':''}</small></span><button type="button" data-compose-file-remove="${i}" aria-label="Remove ${esc(composeFileName(file))}" title="Remove attachment">&times;</button></div>`).join('')}`; }
function addComposeFiles(fileList){ const incoming=[...(fileList||[])]; if (!incoming.length) return; const errors=[]; incoming.forEach(file=>{ const ext=(file.name.split('.').pop()||'').toLowerCase(); if (!COMPOSE_ALLOWED_EXTENSIONS.has(ext)) { errors.push(`${file.name} is not a supported file type.`); return; } if (file.size>COMPOSE_MAX_FILE_BYTES) { errors.push(`${file.name} is larger than 10 MB.`); return; } if (composeFiles.some(existing=>composeFileKey(existing)===`${file.name.toLowerCase()}:${file.size}`)) return; composeFiles.push({ kind:'local', file, name:file.name, filename:file.name, size:file.size, type:file.type }); }); if (composeFiles.length>COMPOSE_MAX_FILES) { composeFiles=composeFiles.slice(0,COMPOSE_MAX_FILES); errors.push('A maximum of 10 attachments can be added.'); } while (composeFiles.reduce((sum,file)=>sum+composeFileSize(file),0)>COMPOSE_MAX_TOTAL_BYTES) { const removed=composeFiles.pop(); errors.push(`${composeFileName(removed)} was removed because attachments exceed 25 MB.`); } renderComposeFiles(); composeFeedback(errors[0]||'', !errors.length); }
function safeFileName(name='attachment'){ return String(name).replace(/[^a-z0-9._-]+/gi,'-').replace(/-+/g,'-').slice(0,120); }
async function composeAuth(){ const { data:{ session } }=await sb.auth.getSession(); if (!session?.access_token || !session.user?.id) throw new Error('Your session expired. Sign in again before uploading attachments.'); return { token:session.access_token, user:session.user }; }
async function uploadComposeAttachments(userId){ if (!composeFiles.length) return { attachments:[], newPaths:[] }; const attachments=[]; const newPaths=[]; for (const entry of composeFiles.slice(0,COMPOSE_MAX_FILES)) { if (entry.kind==='stored' && entry.path) { attachments.push({ name:composeFileName(entry), filename:composeFileName(entry), size:composeFileSize(entry), type:entry.type||'', path:entry.path, category:entry.category||attachmentCategory(entry) }); continue; } const file=entry.file; const path=`${userId}/${todayStr()}/${commId('att')}-${safeFileName(file.name)}`; const { error }=await sb.storage.from('message-attachments').upload(path,file,{ upsert:false, contentType:file.type||undefined }); if (error) { await removeStoredAttachments(newPaths); throw new Error('One or more attachments could not be uploaded. Please try again.'); } newPaths.push(path); attachments.push({ name:file.name, filename:file.name, size:file.size, type:file.type, path, category:attachmentCategory({ name:file.name }) }); } return { attachments, newPaths }; }
async function removeStoredAttachments(paths=[]){ const clean=paths.filter(Boolean); if (!clean.length) return; await sb.storage.from('message-attachments').remove(clean); }
function removedOriginalAttachmentPaths(current=[]){ const currentPaths=new Set(current.map(file=>file.path).filter(Boolean)); return composeOriginalAttachments.map(file=>file.path).filter(path=>path&&!currentPaths.has(path)); }
async function sendComposed(){
  const btn=$('comm-compose-send'); const p=composePayload(); if (!p.to) { composeFeedback('Add a recipient email address.'); return; } if (!p.subject) { composeFeedback('Add a subject line.'); return; } if (!p.body && !composeFiles.length) { composeFeedback('Write a message or add an attachment before sending.'); return; }
  btnLoad(btn,true,'Sending...'); composeFeedback(composeFiles.length?'Uploading attachments...':''); let auth, uploadResult={ attachments:[], newPaths:[] }; try { auth=await composeAuth(); uploadResult=await uploadComposeAttachments(auth.user.id); } catch (err) { btnLoad(btn,false); composeFeedback(err.message); return; } const attachments=uploadResult.attachments;
  let status='sent', errorText=null, externalId=null; try { composeFeedback(attachments.length?'Attachments ready. Sending email...':'Sending email...',true); const res=await fetch('/api/send-message',{ method:'POST', headers:{'Content-Type':'application/json',Authorization:`Bearer ${auth.token}`,'X-Idempotency-Key':composeSendId||commId('send')}, body:JSON.stringify({ to:p.to, cc:p.cc, bcc:p.bcc, subject:p.subject, body:p.body, htmlBody:p.bodyHtml, attachments }) }); const result=await res.json().catch(()=>({})); if (!res.ok) { status='failed'; errorText=result.error || `Send failed (${res.status})`; } else externalId=result.id||null; } catch { status='failed'; errorText='Network error - message not sent.'; }
  if (status==='failed') await removeStoredAttachments(uploadResult.newPaths);
  const row={ client_id:p.clientId || null, direction:'outbound', recipient_email:p.to, subject:p.subject, body:p.body, body_html:p.bodyHtml || null, status, error:errorText };
  if (externalId) row.external_id=externalId; let insert=await sb.from('messages').insert(attachments.length ? { ...row, attachments } : row); if (insert.error && attachments.length && isSchemaError(insert.error)) insert=await sb.from('messages').insert(row);
  btnLoad(btn,false); if (status==='failed') { composeFeedback(errorText || 'Message could not be sent.'); return; } await removeStoredAttachments(removedOriginalAttachmentPaths(attachments)); if (insert.error) toast('Email sent, but it could not be saved to history.'); if (p.draftId) await deleteDraft(p.draftId,{ preserveAttachments:true, render:false }); closeModal(); toast(attachments.length?`Email sent with ${attachments.length} attachment${attachments.length===1?'':'s'}.`:'Email sent.'); await loadMessages(); commActiveTab='inbox'; mailFolder='all'; selectedMessageId=allMessages[0]?.id || null; renderMessages();
}
function replyToMessage(id){ const m=messageById(id); if (!m) return; const subject=/^re:/i.test(m.subject || '') ? m.subject : `Re: ${m.subject || ''}`; const quote=`<div style="margin-top:22px;padding-top:14px;border-top:1px solid #e0dfd8;color:#6b6b64;font-size:13px;line-height:1.6;">On ${esc(fmtDateLong(String(messageDate(m)).split('T')[0]))}, ${esc(messageClientName(m))} wrote:<br>${nl2br(m.body || '')}</div>`; openCompose({ to:messageEmail(m), subject, clientId:m.client_id || null, bodyHtml:`${appendOpsEmailSignature('<p><br></p>')}${quote}`, isReply:true }); }
...
$('page-messages')?.addEventListener('click', e => {
  ...
  if (e.target.closest('[data-folders-toggle]')) { commState.settings.foldersCollapsed=!commState.settings.foldersCollapsed; saveCommState(); renderMessages(); return; }
...
document.addEventListener('click', e => {
  ...
  if (e.target.closest('#comm-compose-dropzone')) { $('comm-compose-files')?.click(); return; }
...
document.addEventListener('change', e => { if (e.target.id==='mail-compose-client') onComposeClientChange(); if (e.target.id==='comm-compose-files') { addComposeFiles(e.target.files); e.target.value=''; } if (e.target.matches('[data-notification-pref]')) { commState.settings.notifications[e.target.dataset.notificationPref]=e.target.checked; saveCommState(); } });
document.addEventListener('dragover', e => { const zone=e.target.closest?.('#comm-compose-dropzone'); if (!zone) return; e.preventDefault(); if (e.dataTransfer) e.dataTransfer.dropEffect='copy'; zone.classList.add('drag-over'); });
document.addEventListener('dragleave', e => { const zone=e.target.closest?.('#comm-compose-dropzone'); if (zone && !zone.contains(e.relatedTarget)) zone.classList.remove('drag-over'); });
document.addEventListener('drop', e => { const zone=e.target.closest?.('#comm-compose-dropzone'); if (!zone) return; e.preventDefault(); zone.classList.remove('drag-over'); addComposeFiles(e.dataTransfer?.files); });
