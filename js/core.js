...
function quoteTemplate(quote, client) {
  return `<!DOCTYPE html><html><head><meta charset="UTF-8">${_pdfStyles()}</head><body><div class="pdf-wrap">
    ...
    <div class="pdf-title">${esc(quote.service_name || 'Quotation')}</div>
    ...
    <div class="pdf-tiles">
      ...
      <div class="pdf-tile"><div class="pdf-tile-lbl">Status</div><div class="pdf-tile-val">${esc(quote.status)}</div></div>
    </div>
    ${_pdfLineItems(quote)}
    ${_pdfFooter()}
  </div></body></html>`;
...
function invoiceTemplate(invoice, client) {
  return `<!DOCTYPE html><html><head><meta charset="UTF-8">${_pdfStyles()}</head><body><div class="pdf-wrap">
    ...
    ${_pdfClientBlock(client)}
    <div class="pdf-title">${esc(invoice.service_name || 'Invoice')}</div>
    ...
    <div class="pdf-tiles">
      ...
      <div class="pdf-tile"><div class="pdf-tile-lbl">Status</div><div class="pdf-tile-val">${esc(invoice.payment_status)}</div></div>
    </div>
    ${_pdfLineItems(invoice)}
    ${_pdfBankingDetails(invoice)}
    ${_pdfFooter()}
  </div></body></html>`;
}
...
function proposalTemplate(proposal, client) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8">${_pdfStyles()}</head><body><div class="pdf-wrap">
    ...
    ${_pdfClientBlock(client)}
    <div class="pdf-title">${esc(proposal.title || 'Proposal')}</div>
    <div class="pdf-subtitle">Proposal &middot; ${compactDate(proposal.proposal_date)}</div>
    ${sections.map(s => `<div class="pdf-section-title">${esc(s.title)}</div><div class="pdf-body-text">${esc(s.body).replace(/\n/g,'<br>')}</div>`).join('')}
    ${investmentHTML}
    ${_pdfFooter()}
  </div></body></html>`;
}
...
function sowTemplate(scope, client, proposal) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8">${_pdfStyles()}</head><body><div class="pdf-wrap">
    ...
    ${_pdfClientBlock(client)}
    <div class="pdf-title">${esc(scope.title || 'Scope of Work')}</div>
    <div class="pdf-subtitle">SOW &middot; ${compactDate(scope.scope_date)}${proposal ? ` &middot; Ref: ${proposal.proposal_number}` : ''}</div>
    ${sections.map((s,i) => `<div class="pdf-section-title">${i+1}. ${esc(s.title)}</div><div class="pdf-body-text">${esc(s.body).replace(/\n/g,'<br>')}</div>`).join('')}
...
function welcomeLetterTemplate(letter, client) {
  const services = (letter.assigned_services || []).map(esc).join(', ') || '—';
  return `<!DOCTYPE html><html><head><meta charset="UTF-8">${_pdfStyles()}</head><body><div class="pdf-wrap">
    ...
    <div class="pdf-body-text">${esc(letter.welcome_message || '').replace(/\n/g,'<br>')}</div>
    <table class="pdf-items" style="margin-bottom:1rem">
      <thead><tr><th>Detail</th><th>Information</th></tr></thead>
      <tbody>
        <tr><td>Plan</td><td>${esc(client.selected_plan || '—')}</td></tr>
        <tr><td>Start Date</td><td>${compactDate(client.project_start_date) || '—'}</td></tr>
        <tr><td>Services</td><td>${services}</td></tr>
      </tbody>
    </table>
    ${letter.important_info ? `<div class="pdf-section-title">Important Information</div><div class="pdf-body-text">${esc(letter.important_info).replace(/\n/g,'<br>')}</div>` : ''}
    ${_pdfFooter()}
  </div></body></html>`;
}

/* Dynamic document template layer. These builders intentionally override the
   earlier per-document PDF functions so every document shares one template. */
